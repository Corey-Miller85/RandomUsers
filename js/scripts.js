...
function populateModal(name) {
    jsonData.forEach((person, index ) => {
        ...
        document.querySelector('#modal-prev').addEventListener('click', (e) => {
            if (currentIndex > 0) {
                currentIndex -= 1;
               const previousCard = visibleCards[currentIndex].querySelector('#name').textContent
               populateModal(previousCard)
            } 
        });
        document.querySelector('#modal-next').addEventListener('click', (e) => {
            if (currentIndex < 11) {
                currentIndex += 1;
               const nextCard = visibleCards[currentIndex].querySelector('#name').textContent
               populateModal(nextCard)
            } 
        });
        }
    });
